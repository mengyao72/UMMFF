...
def positionencoding1D(W, L):#波长#103,4
    ...
    # cache the values so you don't have to do function calls at every pixel#缓存这些值，这样就不必对每个像素进行函数调用
    for el in range(0, L):
        val = 2 ** el
...
def positionencoding2D(W, H, L, basis_function):#空间坐标
