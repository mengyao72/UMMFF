...
class BlindNet(nn.Module):
    def __init__(self, hs_bands, ms_bands, ker_size, ratio):
        ...
        petype = 'sin_cos'
        self.input_1D = torch.from_numpy(positionencoding1D(self.hs_bands, self.ms_bands)).float().to(device)
        self.ssffcn = SSFFcn(self.ms_bands, 5)
    ...
    def forward(self, Y, Z):
        ...
        Phi = self.ssffcn(self.input_1D) ** 2
...
class Blind(DataInfo):
    def __init__(self, ndata, nratio, nsnr=0, blind=False):
        super().__init__(ndata, nratio)
        self.strBR = 'BR.mat'
        self.blind = blind
        if self.blind is False:
            # self.psf, self.srf
            print('using estimate psf and srf!')
            return
