...
class AENet(nn.Module):
    ...
    def __init__(self, hs_bands, ms_bands, edm_num):
        ...
        self.ca2 = ChannelAttention(self.edm_num)
        self.sa2 = SpatialAttention()
        self.ca3 = ChannelAttention(self.edm_num)
    ...
    def forward(self, Yu, Z):
        ...
        Code3 = self.Y_net(E3)#图像融合模块
        Code5 = E3 * self.ca1(Code3)  # 高光谱图像增加通道注意力
        Code4 = self.Z_net(E4)
        Code6 = E4 * self.sa1(Code4)  # 多光谱图像增加空间注意力
        Code5 = rearrange(Code5, 'B c H W -> B (H W) c', H=sz)
        Code6 = rearrange(Code6, 'B c H W -> B (H W) c', H=sz)
        E5 = rearrange(Code5, 'B (H W) C -> B C H W', H=sz)
        E6 = rearrange(Code6, 'B (H W) C -> B C H W', H=sz)
        S = torch.cat([E5, E6], dim=1)
        S = self.S0_net(S)
        S3 = torch.cat([S, E5, E6], dim=1)
        S4 = self.S1_net(S3)
        Highpass = S4.real
...
class UMMFF(DataInfo):
    def __init__(self, ndata, nratio, nsnr=0, psf=None, srf=None, edm_num=48):
        ...
        self.lr_fun = lambda epoch: (1.0 - max(0, epoch + 1 - 200) / 4900)
    ...
    def train(self, max_iter=5000, verb=True, is_save=True):
        # train ...
        ...
        if is_save is True:
            ...
            sio.savemat(self.save_path + self.strX, {'X': X, 't1': train_time})
        pass
...
if __name__ == '__main__':
    ...
    net = UMMFF(ndata=ndata, nratio=nratio, nsnr=nsnr, psf=blind.psf, srf=blind.srf, edm_num=edm_num)
    net.train()
    pass
