...
class SimpleRetention(nn.Module):
    ...
    def forward(self, X, Z):
        ...
        if Z.dtype != self.complex_type:
            Z = torch.complex(Z, torch.zeros_like(Z)).to(self.complex_type)
        ...
        K = (Z @ self.W_K.to(self.complex_type)) * Theta_bar.unsqueeze(0)
        V = Z @ self.W_V.to(self.complex_type)
...
class MultiScaleRetention(nn.Module):
    ...
    def forward(self, X, Z):
        ...
        for i in range(self.heads):
            Y.append(self.retentions[i](X[:, :, i*self.head_size:(i+1)*self.head_size],Z[:, :, i*self.head_size:(i+1)*self.head_size]))
        
        Y = torch.cat(Y, dim=2)
        Y = self.group_norm(Y.reshape(-1, self.hidden_size)).reshape(X.shape)
        Y = (self.swish(X @ self.W_G.to(self.complex_type)) * Y) @ self.W_O.to(self.complex_type)
